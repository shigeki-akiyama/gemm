...
static std::vector<bench_pair> make_benchmarks(int M, int N, int K)
{
    ...
    {
        using blis = blis_th<haswell, register_avx_3_4x3asmpf>;
        blis::intiialize();
    ...
    // 9-4. AVX512 assembly implementation based on BLIS (unroll, prefetch)
    {
        using blis = blis<knl_5x5, register_avx512_5x5asmpf_unroll>;
        blis::intiialize();
        push("blis512_packL3_5x5asmpf_unr", blis::gemm);
    }

    // 9-5. AVX512 assembly implementation based on BLIS (unroll, prefetch)
    {
        using blis = blis<knl_5x5, register_avx512_5x5asmpf_ebcast>;
        blis::intiialize();
